...
@link '''+os.path.splitext(fake_c_name)[0]+'''.obj '''+os.path.splitext(fake_asm_name)[0]+'''.obj /dll /release /subsystem:console /out:'''+fake_dll+''' /MACHINE:X64 /MANIFEST:NO /merge:.rdata=.text
del *.obj
del *.exp
'''
...
if __name__ == '__main__':        
    if len(sys.argv) < 2 or not os.path.isfile(sys.argv[1]):
        ...
        l_export = get_export_name_addr_list(e)
        for name, virt in l_export:
            print name
        filename = os.path.splitext(filename)[0]
        makefile_name = "make.bat"
        res_dir = filename + "_res_dir/"
        fake_c_name = filename + ".c"
        fake_asm_name = filename + "_trampo.asm"
        fake_dll = filename + ".dll"
        original_dll = filename + "_original.dll"
        createdir(res_dir)
